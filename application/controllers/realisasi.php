...
class realisasi extends CI_Controller {	
	...
	public function __construct()
	{
		...
		$this->load->model('datalayer/realisasilayer');
		$this->load->model('datalayer/planninglayer');
		$this->em = $this->doctrine->em;
	}
	
	
	public function listRealisasi()
	{
		...
		if($userlogged){
			...
			$this->twig->display("listtorealisasi",$content);	
		}
		else
			redirect("../");
	}
	...
	public function setWeek()
	{
		...
		if($userlogged){
			...
			$exist = $this->db->get_where("trans_realisasi", $param);
			$num = $exist->num_rows();
	...
	public function formRealisasi()
	{
		...
		if ($userlogged) {
			...
			if($project->status != "Y")
			{
				echo "The Project hasn't approved yet, process cannot be continue <br />";
			...
			$detail = $this->realisasilayer->listDetail($idProject);
			$detplan = $this->planninglayer->listDetail($idProject);
			$ct = $this->contentlayer->contenttext();
			$topnews = $this->contentlayer->topnews();
			$link = $this->contentlayer->linkeks();
			...
			$content = array(
				...
				"detail" => $detail,
				"detplan" => $detplan,
				"page_title" => "Set Realisasi :: EBTKE",
				"log" => $log,
				"ct" =>$ct,
				"news" =>$topnews,
				"links" =>$link,	
			...
			$this->twig->display("realisasifrm", $content);	
		}
	}
	
	
	public function listProjectToRealisasi()
	{
		$i=0;
		...
		$query1 = $this->db->query("SELECT a.*, b.nama_perusahaan, c.kota_kabupaten, d.propinsi, e.nama_jenissurvey
									...
									JOIN mst_propinsi d ON c.propinsi_id = d.propinsi_id where a.status='Y' $param");	
		...
		$query2 = $this->db->query("SELECT a.*, b.nama_perusahaan, c.kota_kabupaten, d.propinsi, e.nama_jenissurvey
									...
									JOIN mst_kota c ON c.kota_id=a.kota_id
									JOIN mst_jenissurvey e ON e.id_jenissurvey = a.id_survey
									JOIN mst_propinsi d ON c.propinsi_id = d.propinsi_id where a.status='Y' $param order by $sidx $sord limit $start , $limit");							
